...
# отдельный запрос getPrices. Клиент ограничен 1 запросом в секунду, таймаут
# одного запроса 25с, а API отвечает медленно — на страну уходит ~35с, полный
# проход занимает около двух часов. С потолком 600с проход обрывался на 15-й
# стране, и цены остальных 187 переставали обновляться совсем (проверено на бою
# 20.08.2026). Потолок считаем от худшего случая 202 x 26с = 5252с плюс запас.
#
# Задача — кандидат на оптимизацию: при интервале в 30 минут двухчасовой проход
# означает, что она работает практически непрерывно.
