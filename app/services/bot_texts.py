...
<tg-emoji emoji-id="5258215846450305872">📲</tg-emoji>Принять СМС - номер на 15 минут для конкретного сервиса
<tg-emoji emoji-id="5258419835922030550">📞</tg-emoji>Длительная аренда - аренда номера на 1-30 дней для любых сервисов
...
- Если номер принял SMS, услуга считается оказанной и не подлежит возврату
...
<i>- Номер активен до {active_until} мск
- Срок жизни номера 15 минут.
- Если СМС не поступит, деньги вернутся на баланс в боте
- После истечения аренды повторно использовать номер невозможно</i>

<b><tg-emoji emoji-id="5447644880824181073">⚠️</tg-emoji> Если на номер поступил код и после ввода кода сервис выдал ошибку, обязательно сделайте полноэкранный скриншот</b>
"""
...
✅ СМС доставлено
<b>Сообщение:</b> <code>{sms_text}</code>
...
SERVICE_INFO_TELEGRAM_SMS_RECEIVED = """
<b>Ваш номер</b> <code>+{phone}</code> (Кликабельно)
...
MY_RENT_EMAIL = """
<b>Ваш почтовый ящик <tg-emoji emoji-id="5197474438970363734">⤵️</tg-emoji></b>
{email}

<i>Ожидаем письмо...</i>

<tg-emoji emoji-id="5447644880824181073">⚠️</tg-emoji> Арендован до: <b>{expire_at}</b>
"""

CONFIRM_RENT_EMAIL = """
<b>Вы собираетесь арендовать почтовый ящик <tg-emoji emoji-id="5197474438970363734">⤵️</tg-emoji></b>
...
<i>Арендованный ящик подходит для приёма писем для сервисов регистрации. В отличие от временной почты, вероятность приёма писем выше.</i>
"""
...
SELECT_FAVORITES_TITLE = '<tg-emoji emoji-id="5296348778012361146">🏷</tg-emoji> Избранные сервисы'
BACK_TO_SERVICES_BTN = 'К списку сервисов'
NO_FAVORITES_POPUP = 'У вас пока нет избранных сервисов'
REQUEST_ANOTHER_CODE = 'Выбрать новый номер'
RECEIVE_ANOTHER_SMS_TO_NUMBER = 'Запросить повторное SMS'
