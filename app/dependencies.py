    ...
    "connections": {
        "default": {
            ...
            "credentials": {
                ...
                #
                # Канал до БД периодически деградирует (16.08.2026 задержка выросла
                # с 36 до 318 мс с потерей пакетов). На таком канале дорого именно
                # УСТАНОВИТЬ соединение, поэтому держим тёплый резерв и не даём
                # пулу закрывать их по короткому таймауту простоя.
                "minsize": 3,
                "maxsize": 10,
                "max_queries": 10000,
                "max_inactive_connection_lifetime": 600.0,
                # Потолок на запрос: без него оборванные соединения оставляли
                # запросы висеть по 15 минут, занимая backend'ы на сервере БД.
